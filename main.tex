...
footprint.  For Gene Ontology datasets with $C > 4{,}000$, storing the
matrix alone costs $>65$~MB ($C^2 \times 4$ bytes), and applying it to
a batch of logits materializes a batch$\times C\times C$ tensor in
double precision ($\approx$8.7\,GB at batch 64)---infeasible on GPU
alongside model parameters and activations.  We address this with the
\textbf{Block-Diagonal R-Matrix}, a sparse $O(C+E)$ method that
achieves \textbf{zero hierarchy violations without ever materializing
the dense ancestor matrix}.
...
\begin{table}[htbp]
\centering
\caption{Block-Diagonal R-Matrix on Gene Ontology datasets (9 datasets,
...
benchmark suite.  The dense R-matrix costs $N^2 \times 4$ bytes
($\approx$65~MB for $N{=}4{,}126$) to store, and applying it to a
batch materializes a batch$\times N\times N$ float64 tensor
