...
dense $O(C^2)$ matrix is infeasible, we introduce the
\textbf{Sparse R-Matrix}---a sparse approximation that
reconciles scores via graph traversal in $O(C{+}E)$ memory, eliminating
...
(avg +0.0055) while the dense R-matrix would require $>$65~MB to
store and a batched $C\times C$ constraint step that exceeds GPU
memory.  Our sparse approximation achieves zero
...
R-matrix implementation} for the graph-based version that preserves
the same semantics without materializing the dense matrix.
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{Sparse R-matrix implementation}: a graph-based
    $O(C{+}E)$ approximation that extends the R-matrix constraint to
    large DAGs (4,000+ classes) where the dense $O(C^2)$ matrix is
    infeasible.  Implemented directly on graph structure, it preserves
    the semantics of the R-matrix constraint while avoiding dense
    ancestor-matrix materialization.  Validated
    on Gene Ontology benchmarks with zero hierarchy violations.
  ...
  \item Open-source release with GPU acceleration (14$\times$
    training speedup).  Code: \url{https://github.com/sette/hmc-torch},
    package: \texttt{pip install hmc-torch} (\url{https://pypi.org/project/hmc-torch/}).
\end{itemize}
...
the sparse R-matrix implementation for large label spaces. The graph is
represented directly as a \texttt{NetworkX} DAG, so label structure
stays explicit instead of being collapsed into the dense R-matrix.
...
\end{tikzpicture}
\caption{Hierarchy graph used by the sparse R-matrix implementation. The example uses ArXiv and WOS label fragments to mirror the datasets evaluated in the paper.}
\label{fig:sparse_graph}
\end{figure}

Figure~\ref{fig:sparse_flow} summarizes the algorithmic flow: build the
graph once, recover levels with BFS, and reconcile scores bottom-up
along the real parent-child edges only. This is the \textit{sparse}
\texttt{NetworkX}-based implementation of the R-matrix constraint
itself, not the dense baseline: the $O(C^2)$ cost belongs only to
materializing and applying the full dense R-matrix, while our
implementation stays at $O(C+E)$ memory and avoids dense
materialization.
...
\node[box] (build) {1. Build graph\\\texttt{NetworkX}};
\node[box, below=of build] (bfs) {2. BFS levels\\+ node index};
\node[box, below=of bfs] (prop) {3. Bottom-up\\max propagation};
\node[note, below=of prop] (cost) {\textbf{Dense baseline:} $O(C^2)$\\\textbf{Sparse impl.:} $O(C+E)$};
\draw[arrow] (build) -- (bfs);
\draw[arrow] (bfs) -- (prop);
\draw[arrow] (prop) -- (cost);
\end{tikzpicture}
\caption{Sparse R-matrix flow. The graph is built once with \texttt{NetworkX}, levels are derived by BFS, and reconciliation traverses only the actual parent-child edges. This figure describes the sparse implementation of the R-matrix constraint; the $O(C^2)$ term refers only to the dense baseline, while the sparse implementation uses $O(C+E)$ memory.}
...
datasets with $C > 4{,}000$, storing the dense matrix alone costs
>$65$~MB ($C^2 \times 4$ bytes), and applying it to a batch of logits
materializes a batch$\times C\times C$ tensor in double precision
($\approx$8.7\,GB at batch 64)---infeasible on GPU alongside model
parameters and activations.  We address this with the \textbf{sparse
R-matrix implementation}, an $O(C+E)$ method that achieves
\textbf{zero hierarchy violations without ever materializing the dense
ancestor matrix}.
...
The sparse R-matrix implementation (Section~\ref{sec:sparse}) is
implemented entirely from the graph adjacency list, without
constructing any dense matrix.  The \texttt{BlockDiagonalR} class in
...
$O(C + E)$ memory.  For cellcycle\_GO
($C{=}4{,}126$, $E{=}5{,}827$), this reduces memory from 65~MB to
$\sim$46~KB ($E \times 8$~B index pairs, $>1{,}400\times$ reduction).
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{R-matrix constraint.} The hierarchical consistency
    ...
    descendants.
  \item \textbf{Sparse R-matrix implementation.} The graph-based
    version of the same constraint, implemented directly on the
    hierarchy edges without materializing the dense matrix.
  \item \textbf{BFS levels.} The depth ordering recovered from the root
    using Breadth-First Search. We use it to process the hierarchy from
    leaves to root during reconciliation.
  \item \textbf{Bottom-up max propagation.} The reconciliation rule
    that updates each parent with the maximum score observed among its
    children and itself.
  \item \textbf{Dense baseline.} The classical matrix-based
    implementation used only as the reference point for complexity
    comparisons.
\end{itemize}
...
3,500--4,504 classes).  Dense R is infeasible on GPU: the batched
$C\times C$ float64 constraint step exceeds memory at batch 64.
...
\multicolumn{6}{l}{\small Dense R would OOM at batch 64: the batched $C\times C$ float64 step peaks at $\approx$10.4\,GB.} \\
...
($\approx$8.7\,GB at batch 64), which cannot fit in GPU memory
alongside model parameters and activations; the sparse approximation
...
dense R-matrix cannot fit in GPU memory for GO datasets---storing it
costs $>65$~MB and the batched constraint step materializes a
batch$\times C\times C$ float64 tensor ($\approx$10.4\,GB at batch 64
on the largest dataset)---while our Block-Diagonal approximation runs
successfully on all 9 datasets.  Combining sparse R with ontology embeddings and GCN label
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{Sparse R-Matrix}: a sparse $O(C{+}E)$ approximation
