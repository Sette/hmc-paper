...
\textbf{28 datasets} spanning 7 domains: ArXiv, WOS, AAPD, RCV1-V2,
EUR-Lex (scientific, news, legal text), FunCat and Gene Ontology
(genomics), Enron (email), Diatoms (microscopy), and ImCLEF (medical
imaging).
...
The R-matrix, introduced by~\citet{giunchiglia2018hmc}, is the
...
R-Matrix}, a sparse $O(C+E)$ approximation.

\textbf{Key observation.}  The reconciliation in
Eq.~\ref{eq:reconcile} requires only the \emph{graph adjacency}
(parent-child edges), not the full transitive closure.  We can
propagate scores upward one hop at a time:

\begin{equation}
\forall \text{ node } c,\; \forall p \in \text{parents}(c):\;
p \leftarrow \max(p, c)
\label{eq:sparse_reconcile}
\end{equation}

\textbf{Algorithm.}
\begin{enumerate}[leftmargin=*,nosep]
  \item Compute node levels via BFS from root(s) --- $O(C+E)$.
  \item For each level from deepest to shallowest:
    \begin{itemize}
      \item For each node $c$ at this level:
        \item For each parent $p$ of $c$:
          \item $p \leftarrow \max(p, c)$
    \end{itemize}
  \item For DAGs (multiple parents possible), repeat steps 2--3 up to
    $\text{max\_depth}$ times.  Each pass propagates scores one hop
    upward; after $L$ passes, all ancestors within $L$ hops are
    updated.  Convergence is guaranteed because the longest path in
    the DAG has length $\leq \text{max\_depth}$.
\end{enumerate}
...
first time---R-matrix-constrained training and inference on the full
Gene Ontology benchmark suite.

\textbf{Equivalence.} For tree hierarchies, one pass of
Eq.~\ref{eq:sparse_reconcile} produces exactly the same result as the
dense R-matrix reconciliation (Eq.~\ref{eq:reconcile}), since there is
exactly one path from any node to the root.  For DAGs, the multi-pass
variant converges to the same fixed point as the dense transitive
closure, because the max operation is associative and commutative:
propagating through direct parents iteratively is equivalent to
propagating through all ancestors simultaneously.
...
domains: \textbf{AAPD} (arXiv papers, 54 labels, 2-level tree),
\textbf{RCV1-V2} (Reuters news, 103 topics, 4-level tree), and
\textbf{EUR-Lex 57K} (EU legislation, 1,998 EUROVOC concepts,
up to 8 levels deep).

\begin{table}[h]
\centering
\caption{Micro-F1 on new text HMC benchmarks (frozen SPECTER2 + R).
Preliminary results; $^\dagger$RCV1-V2 pending download.}
\label{tab:new_text}
\begin{tabular}{lccc}
\toprule
...
\textbf{EUR-Lex 57K.} With 2,901 labels and frozen SPECTER2 embeddings
